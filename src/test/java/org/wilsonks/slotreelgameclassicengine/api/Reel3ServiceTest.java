...
@SpringBootTest
class Reel3ServiceTest {

    @Autowired
    private Reel3Service reel3Service;

    @Test
    void testReel3SymbolsForAllSpreadIndexes() {
        String[] strip = {
                "JD", "RS", "1B", "2X", "T6", "3B", "BE", "BS", "T5", "2B",
                "RU", "T4", "WI", "SS", "T2", "1B", "WH", "T5", "3B", "2X",
                "T1", "BE", "2B", "CH", "BS", "T5", "GC", "1B", "WI", "T4",
                "MA", "T3"
        };
        ...
        for (int stopIndex = 0; stopIndex < spread.length; stopIndex++) {
            ...
            for (int inputIndex : inputIndexes) {
                int actualStopIndex = reel3Service.getReelStopIndex(0, inputIndex);
                assertEquals(stopIndex, actualStopIndex,
                        String.format("Input index %d should map to stop index %d", inputIndex, stopIndex));

                String actualSymbol = reel3Service.getSymbolAtIndex(actualStopIndex);
